...
const CONTEXT_KEY="tdt_extension_google_auth_context_v4";
const status=document.getElementById("status"),loginButton=document.getElementById("googleLogin"),cancelButton=document.getElementById("cancel"),spinner=document.querySelector(".spinner");
const CONFIG=globalThis.TDT_CONFIG||{};
function setStatus(m,t=""){status.textContent=m;status.className=`status${t?` ${t}`:""}`;}
function readContext(){const p=new URLSearchParams(location.search),c={extensionId:String(p.get("extensionId")||"").trim(),nonce:String(p.get("nonce")||"").trim().toLowerCase()};if(c.extensionId!==EXPECTED_EXTENSION_ID||!/^[a-f0-9]{64}$/.test(c.nonce))throw new Error("Liên kết đăng nhập không hợp lệ. Hãy mở lại từ extension.");sessionStorage.setItem(CONTEXT_KEY,JSON.stringify(c));return c;}
